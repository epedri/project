...
def main():
        ...
        with st.form(key="submit_form"):
            ...
            if st.session_state["submitted"]:
                for el in list_names:
                    st.session_state[el.split("_")[0] + "_final"] = st.session_state["el"]
    ...
    if st.session_state["submitted"]: #or st.session_state["mapped"]:
        ...
        tot = []
        for val in list_names:
            tot.append((val.split("_")[0] + "_quality", st.session_state[el.split("_")[0] + "_final"]))
        st.session_state["trying"] = user_importance(data, tot)
        st.session_state["map_base"] = st.session_state["trying"].explore(column='result', tiles='CartoDB dark_matter', cmap="RdYlGn", vmin=0, vmax=5,
